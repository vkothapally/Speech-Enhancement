...
class WpeMethod(object):
    ...
    """
    def __init__(self, mic_num, out_num, order=10):
...
def wpe_dereverb(x, samplerate):
    
    """Main script for WPE dereverberation
    ...
    wpe = WpeMethod(1, 1, 30)
    xlen = len(x)
    x = np.reshape(x,(1, len(x)))
    drv_data = wpe.run_offline(x)
    drv_data = drv_data.flatten()
    if len(drv_data) >= xlen:
        drv_data_final = np.split(drv_data,[xlen])[0]
    elif len(drv_data) < xlen:
        drv_data_final = np.zeros((xlen,))
        drv_data_final[:len(drv_data)] = drv_data
    drv_data_final = drv_data_final*np.max(np.abs(x))/np.max(np.abs(drv_data_final))
    return drv_data_final
